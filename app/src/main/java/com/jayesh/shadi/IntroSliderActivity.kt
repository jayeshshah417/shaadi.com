...
class IntroSliderActivity : AppCompatActivity(),IntrosliderViewPagerAdapter.ProfileCallbacks ,
    ModelCallbacks {
    ...
    private lateinit var viewPagerAdapter:IntrosliderViewPagerAdapter
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tablayout.addOnTabSelectedListener(object :TabLayout.OnTabSelectedListener{
            override fun onTabSelected(tab: TabLayout.Tab?) {
                if(tab?.position == profileList.size-1){
                    page = page+1
                    dataViewModel.getData(curr_position_new, 10, profileList)
                }
        ...
        viewPagerAdapter =
            IntrosliderViewPagerAdapter(ArrayList(), applicationContext,this)
        ...
        dataViewModel.getData(curr_position_new,10,profileList).observe(this,{
            if(profileList.size==0 || (it.get(it.size-1).getEmail() != profileList.get(profileList.size-1).getEmail())) {
                ...
                viewPagerAdapter.notifyDataSetChanged()
                if (firstLoad) {
                    viewPager2.setCurrentItem(curr_position)
                    firstLoad = false
